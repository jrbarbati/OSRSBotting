...
public class Walker
{
    ...
    public Tile getNextTile(Tile[] t)
    {
        ...
        for (int i = t.length - 1; i >= 0; i--)
        {
            ...
            {                                    //This is the index at which the suggested next tile resides
                if (i + 1 <= t.length - 1 && nextTile.distanceTo(p) < 3)
                {  //If we're not at the end of the path and we're very close to the suggested next tile
    ...
    public boolean handleObstacle(Tile[] t)
    {
        ...
        for (GameObject go : ctx.objects.select(5).name(Pattern.compile("(staircase)|(ladder)|((.*)?door)|(gate(.*)?)", Pattern.CASE_INSENSITIVE)))
        {                            //Changed to filter by name else all the filtering in the next part takes much too long to be considered effective..
            ...
            {          //we can now assume that we need to go up or down .. not flawless whatsoever
